...
模型给出的核心结论是：**少冰并不等于少稀释**。当冰量不足时，冰会很快全部融化，但饮料仍然可能不够冷；当冰量足够时，可乐能更快进入低温区，而融化量不会随加冰量线性增加。以 $cola_mass_g g、$cola_initial_c deg C 的可乐为例，若希望在几分钟内获得接近冰镇的口感，模型建议使用约 **100-150 g 大冰块**，而不是只放一两小块。
...
- 可乐质量：$cola_mass_g g
- 可乐初温：$cola_initial_c deg C
- 环境温度：$ambient_c deg C
- 冰块初温：$ice_initial_c deg C
- 可乐比热容：$cola_cp J/(g K)
- 冰的比热容：$ice_cp J/(g K)
- 冰的熔化潜热：$ice_fusion J/g

这个模型忽略了可乐中糖、酸、二氧化碳对热容的细小修正，也没有直接模拟气泡逸出。它的目标不是给出唯一真理，而是解释主要趋势。

## 2. 封闭体系热量守恒

如果只考虑热量守恒，把 $cola_mass_g g 可乐从 $cola_initial_c deg C 降到 4 deg C，需要移走的热量约为：

```text
Q = m c Delta T = $cola_mass_g x $cola_cp_short x ($cola_initial_c - 4)
  = $heat_removed J
```
...
这个分数不是心理物理学定律，只是为了帮助比较方案。在基准参数下，5 分钟饮用窗口的最高分出现在约 **$best5_ice_g g** 冰，10 分钟窗口的最高分出现在约 **$best10_ice_g g** 冰。
...
对于一杯 $cola_mass_g mL、常温 $cola_initial_c deg C 的可乐，比较合理的实用建议是：
...
1. 准备 6 杯各 $cola_mass_g mL 可乐，初温统一到 $cola_initial_c deg C。
